...
static int decode_output_int8(const int8_t *predictions, box_data boxes[], int nb_box_max)
{
    ...
    //changed to predictions are in an array [bg, palm, two, left, top, right, bottom]
    int cls_offset = 1; //palm
    int cls2_offset = 2; //two added this
    int reg_offset = 3; //box coords
    ...
    {//changed from 1 score to 2 scores
        int8_t score_palm = predictions[cls_offset];
        int8_t score_two  = predictions[cls2_offset];
        score = (score_palm > score_two) ? score_palm : score_two;
        if (score >= threshold)
        {
            curr_box.label = (score_two > score_palm) ? 2 : 1;
        ...
        cls_offset = cls_offset + 7; //changed to 7.. was originally = cls_offset+6
        cls2_offset = cls2_offset + 7; //added this
        reg_offset = reg_offset + 7; //same here
    }
...
int32_t ULTRAFACE_ProcessOutput(const mpp_inference_cb_param_t *inf_out, box_data* final_boxes, int nb_box_max)
{
    ...
    if (inf_out->inference_type == MPP_INFERENCE_TYPE_TFLITE) {
        preds_int = (int8_t * ) inf_out-> out_tensors[0]->data; /* [1, 4420, 7] matrix */ //changed from ,6 to ,7
        if (preds_int == NULL) {
