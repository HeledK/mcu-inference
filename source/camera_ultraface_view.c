...
#include "models/ultraface_slim_quant_int8/ultraslim_int8_converted.h"

#else
#include "models/ultraface_slim_quant_int8/ultraslim_int8.h"
#endif

#if defined(APP_USE_NEUTRON16_MODEL) && (HAL_TFLM_TENSOR_ARENA_SIZE_KB > 254) //change this to <254
...
void boxes_to_rects(box_data boxes[], uint32_t num_boxes, uint32_t max_boxes, mpp_labeled_rect_t *rects)
{
    ...
    const char* labels[] = {"bg", "palm", "two"};  // map labels

    uint8_t label_size;  // declare once at the top
    ...
    for (uint32_t i = 0; i < num_boxes && box_counter < max_boxes; i++) {
        ...
        // Use label from box_data
        label_size = sizeof(rects[box_counter].label);
        strncpy((char *)rects[box_counter].label, labels[boxes[i].label], label_size - 1);
        rects[box_counter].label[label_size - 1] = '\0';
...
int mpp_event_listener(mpp_t mpp, mpp_evt_t evt, void *evt_data, void *user_data)
{
    ...
    case MPP_EVENT_INFERENCE_OUTPUT_READY:
        ...
        if (Atomic_CompareAndSwap_u32(&app_priv->accessing, 1, 0) == ATOMIC_COMPARE_AND_SWAP_SUCCESS) {
            ...
            snprintf((char *)app_priv->labels[0].label,
                    sizeof(app_priv->labels[0].label),
                    "%uKB | %dms",
                    (unsigned)(model_data_len / 1024),
                    app_priv->inference_time_ms);
            /* count valid results */
...
static void app_task(void *params)
{
    ...
    PRINTF("Model size: %u bytes (%.1f KB)\r\n",
        (unsigned)model_data_len, model_data_len / 1024.0f);
    PRINTF("Flash used: %u / 2097152 bytes (%.1f%%)\r\n",
        (unsigned)model_data_len, (model_data_len * 100.0f) / 2097152.0f);
    PRINTF("Tensor arena: %d KB\r\n", HAL_TFLM_TENSOR_ARENA_SIZE_KB);
    PRINTF("==================\r\n");
    ...
    for (;;) {
        ...
        if (Atomic_CompareAndSwap_u32(&user_data.accessing, 1, 0)) {
            ...
            //added
            PRINTF("Free heap: %u bytes (low-water: %u)\r\n",
            ...
            if (user_data.detected_count <= 0) {
                PRINTF("ultraface : no hand gesture detected\r\n");
            } else {
                for (int i = 0; i < NUM_BOXES_MAX; i++)
                {
                    if (user_data.boxes[i].area > 0)
                    {//replaced ultraface detection label
                        const char* labels[] = {"bg", "palm", "two"};
                        PRINTF("ultraface : box %d label %s score %d(%%)\r\n", i,
                            labels[user_data.boxes[i].label], (int)(user_data.boxes[i].score * 100.0f));
                    }
                }
            }
